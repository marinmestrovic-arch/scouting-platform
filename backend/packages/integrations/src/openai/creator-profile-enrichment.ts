...
function buildPrimaryPrompt(input: z.output<typeof inputSchema>): string {
  return [
    ...
    "Country/Region means the creator's own base/location; do not infer it from audience geography, language, or places mentioned in content.",
    "Language means the dominant spoken content language; do not infer it from country or YouTube interface metadata alone.",
    "Choose Influencer Vertical only from repeated creator-led content, not incidental keywords or one-off videos.",
...
function buildClassificationPrompt(input: z.output<typeof inputSchema>): string {
  return [
    ...
    "Country/Region means the creator's own base/location, never audience geography or a language-based guess.",
    "Language means the dominant spoken content language, supported by the creator evidence.",
    "Choose Influencer Vertical only from repeated creator-led content, not incidental keywords.",
