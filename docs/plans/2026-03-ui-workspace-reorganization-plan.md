...
- This is a UI-led redesign inspired by `frontend_arch_platform`; run target persistence was later added so Dashboard coverage can use stored creator-count goals without fabricating frontend-only state.
...
3. Build `/new-scouting` as a single-panel form based on the mock. Run name, target, and prompt are live and persist to the current run creation flow. `Campaign`, `week`, `source mode`, `brief upload`, `platform`, and threshold fields render as clearly disabled planned controls, with `scouting + database` shown as the fixed current backend behavior.
4. Build `/dashboard` as a run operations board. Each row deep-links to `/database?tab=runs&runId=...` and exposes `Export CSV` / `Import to HubSpot` actions for the full run. The requested week/client/manager filters and metadata columns stay visible as backend-dependent scaffolds, while `Target` and `Coverage` use the stored run target plus live result counts.
...
- No real campaign, market, week, or manager metadata exists in current contracts, so those UI elements must not fabricate persisted state.
